...
public class CaseTestCPE0302 {
	...
	By alertRegisterCorrectLocator = By.cssSelector("div[class=\"alert alert-warning alert-dismissible fade show\"]");
	...
	public void test() {
		...
		if(driver.findElement(registroTextLocator).isDisplayed()) {
			driver.findElement(userNameLocator).sendKeys("Guillermina");
			
			driver.findElement(btnRegisterLocator).click();
			
			assertTrue(driver.findElement(alertRegisterCorrectLocator).getText().contains("Por favor revisa el campo nombre, máximo de caracteres 10"));
		} 
	}
